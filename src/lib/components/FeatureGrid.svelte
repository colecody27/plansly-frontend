<section class="section-spacing px-6 lg:px-16">
  <div class="space-y-3 max-w-2xl">
    <p class="text-sm uppercase tracking-[0.2em] text-primary">Hosting made easy</p>
    <h2 class="text-3xl lg:text-4xl font-semibold">Everything you need to be the host with the most.</h2>
    <p class="text-base text-base-content/70">
      From auto-generated invites to live budget tracking, Plannit helps you keep the energy high
      and the logistics calm.
    </p>
  </div>
  <div class="grid gap-6 mt-10 lg:grid-cols-3">
    <div class="card bg-base-100 border border-base-200 shadow-sm">
      <figure class="px-6 pt-6">
        <img
          class="rounded-2xl h-40 w-full object-cover"
          src="https://images.unsplash.com/photo-1521790366320-5cb7462d95f5?auto=format&fit=crop&w=800&q=80"
          alt="Create plan"
        />
      </figure>
      <div class="card-body">
        <h3 class="card-title">Create pages in seconds</h3>
        <p class="text-sm text-base-content/70">Spin up a plan with themes, cover photos, and auto budgets.</p>
      </div>
    </div>
    <div class="card bg-base-100 border border-base-200 shadow-sm">
      <figure class="px-6 pt-6">
        <img
          class="rounded-2xl h-40 w-full object-cover"
          src="https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?auto=format&fit=crop&w=800&q=80"
          alt="Invite"
        />
      </figure>
      <div class="card-body">
        <h3 class="card-title">Send the link</h3>
        <p class="text-sm text-base-content/70">Invite friends anywhere. No app required for guests.</p>
      </div>
    </div>
    <div class="card bg-base-100 border border-base-200 shadow-sm">
      <figure class="px-6 pt-6">
        <img
          class="rounded-2xl h-40 w-full object-cover"
          src="https://images.unsplash.com/photo-1496307042754-b4aa456c4a2d?auto=format&fit=crop&w=800&q=80"
          alt="Payments"
        />
      </figure>
      <div class="card-body">
        <h3 class="card-title">Secure the bag</h3>
        <p class="text-sm text-base-content/70">Collect buy-ins up front and track who has paid instantly.</p>
      </div>
    </div>
  </div>
</section>
